    ...
    node('mypod') {
        ...
        stage ('Docker Build and Push') {
            ...
            container ('docker') {
                ...
                sh "docker build -t servicea:4.0.0 serviceA/."
                sh "docker tag servicea:4.0.0 ${registryIp}/servicea:4.0.0"
                sh "docker push ${registryIp}/servicea:4.0.0"
        ...
        stage ('Helm Deploy') {
            container ('helm') {
                ...
                sh("/helm upgrade --install --wait --set image.repository=gcr.io/flawless-mason-258102,image.tag=4.0.0 servicea helm/serviceA/ --namespace microservices")
