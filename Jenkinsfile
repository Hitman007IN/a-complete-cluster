    ...
    node('mypod') {
        ...
        stage ('Helm Deploy') {
            container ('helm') {
                ...
                sh("/helm upgrade --install --wait --set image.repository=gcr.io/qwiklabs-gcp-00-fc09b4cc0edd,image.tag=1.0.0 ./helm/serviceA/ --namespace dev")
                sh("/helm upgrade --install --wait --set image.repository=gcr.io/qwiklabs-gcp-00-fc09b4cc0edd,image.tag=1.0.0 ./helm/serviceB/ --namespace dev")
            }
        }
    }
}
